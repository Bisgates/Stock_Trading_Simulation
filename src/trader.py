...
class TradeManager:
    ...
    def store_transactions(self):
        with open('transactions.csv', 'w', newline='') as csvfile:
            fieldnames = ['Buy Date', 'Sell Date', 'Buy Price', 'Sell Price', 'Return Rate', 'Hold Duration', 'Notes']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()

            total_return = 1
            for buy, sell in zip(self.transactions['buy'], self.transactions['sell']):
                writer.writerow({'Buy Date': buy[0], 'Sell Date': sell[0], 'Buy Price': f"{buy[1]:.2f}", 'Sell Price': f"{sell[1]:.2f}", 'Return Rate': f"{sell[2]:.2%}", 'Hold Duration': sell[3]})
